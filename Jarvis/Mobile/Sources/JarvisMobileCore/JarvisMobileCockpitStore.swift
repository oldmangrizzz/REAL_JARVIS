...
@MainActor
public final class JarvisMobileCockpitStore: ObservableObject {
    ...
    public let role: JarvisDeviceRole
    ...
    private let voice: JarvisMobileVoiceCloneEngine
    ...
    public init(role: JarvisDeviceRole, bundle: Bundle = .main) {
        self.role = role
        let configuration = JarvisHostConfiguration.load(from: bundle, role: role)
        let registration = JarvisDeviceIdentity.registration(role: role, bundle: bundle)
        self.configuration = configuration
        self.registration = registration
        self.convex = JarvisConvexSyncClient(configuration: configuration)
        self.voice = JarvisMobileVoiceCloneEngine(bundle: Bundle(for: BundleToken.self))
    }
    ...
    public func start() async {
        ...
        do {
            ...
            try await convex.heartbeat(registration: registration, state: connectionState, pushToken: UserDefaults.standard.string(forKey: "jarvis.mobile.push-token"))
        } catch {
    ...
    private func receive(message: JarvisTunnelMessage) {
        ...
        case .response:
            if let response = message.response {
                latestResponse = response.spokenText
                if let snapshot = response.snapshot {
                    state = JarvisSharedState(snapshot: snapshot, thoughts: snapshot.recentThoughts, signals: snapshot.recentSignals, pendingPushDirectives: state.pendingPushDirectives)
                }
                if let path = response.outputPath {
                    lastSpeechPath = path
                }
            }
        case .push:
    ...
    private func speak(line: String) async {
        do {
            let path = try await voice.play(text: line)
            lastSpeechPath = path.path
            voiceState = "speaking"
        } catch {
            voiceState = "failed: \(error.localizedDescription)"
        }
