...
public final class DeltaXTTSBackend: TTSBackend, CanonVerifiedBackend {
    ...
    public init(refClipSHA: String, session: URLSession = .shared) throws {
        ...
        guard let bearer = env["JARVIS_TTS_BEARER"], !bearer.isEmpty else {
            throw JarvisError.configurationError("JARVIS_TTS_BEARER not set")
        }
    ...
    public func synthesize(
        ...
        let task = Task {
            do {
                resultData = try await self.synthesizeAsync(
                    text: text,
        ...
        guard let data = resultData else {
            throw JarvisError.processFailure("DeltaXTTSBackend: no data returned")
        }
    ...
    /// Actual async synthesis logic with proper timeouts and retries.
