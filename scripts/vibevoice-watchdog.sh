#!/bin/zsh
# Jarvis VibeVoice watchdog.
#
# What it does (every invocation):
#   1. Asks GCP for jarvis-vibevoice-t4 status + current external IP.
#   2. If the VM is missing/stopped/terminated, re-runs gcp-up.sh.
#   3. Hits /healthz. If ok, writes the current env (URL + bearer) to
#      ~/.jarvis/vibevoice.env and (if changed) restarts the local
#      voice-bridge so Obsidian picks up the new IP without user action.
#   4. Logs to ~/.jarvis/vibevoice-watchdog.log (rotated at 5MB).
#
# Intended to be driven by launchd (com.grizz.jarvis.vibevoice.watchdog)
...
ZONE="us-central1-a"
INSTANCE="jarvis-vibevoice-t4"
REPO="/Users/grizzmed/REAL_JARVIS"
ENV_FILE="${HOME}/.jarvis/vibevoice.env"
BRIDGE_ENV="${HOME}/REAL_JARVIS/.jarvis/voice-bridge.env"
LOG_FILE="${HOME}/.jarvis/vibevoice-watchdog.log"
...
if [[ -z "$STATUS" || "$STATUS" != "RUNNING" ]]; then
  log "VM state=${STATUS:-missing}; calling gcp-up.sh"
  (cd "$REPO" && GCP_PROJECT_ID="$PROJECT" /bin/bash services/vibevoice-tts/deploy/gcp-up.sh) >> "$LOG_FILE" 2>&1
  sleep 5
  STATUS_LINE="$("$GCLOUD" compute instances describe "$INSTANCE" \
    --project="$PROJECT" --zone="$ZONE" \
    --format='value(status,networkInterfaces[0].accessConfigs[0].natIP)' 2>/dev/null)"
...
BEARER_CACHE="${HOME}/.jarvis/vibevoice-bearer"
if [[ ! -s "$BEARER_CACHE" ]]; then
  BEARER="$("$GCLOUD" secrets versions access latest --secret=jarvis-vibevoice-bearer --project="$PROJECT" 2>/dev/null)"
  if [[ -n "$BEARER" ]]; then
    umask 077
