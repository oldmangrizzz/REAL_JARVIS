...
F5_IDLE_SECONDS so the GCP spot instance can deallocate.
"""
...
from synthesizer import F5Synthesizer

BEARER_TOKEN = os.environ.get("F5_BEARER")
if not BEARER_TOKEN:
    print("FATAL: F5_BEARER env var not set; refusing to start.", file=sys.stderr)
    sys.exit(2)

IDLE_SECONDS = int(os.environ.get("F5_IDLE_SECONDS", "1800"))
RETURN_FORMAT = os.environ.get("F5_RETURN_FORMAT", "wav").lower()  # "wav" | "json"

app = FastAPI(title="JARVIS F5-TTS", version="1.0.0")
...
class SynthesizeRequest(BaseModel):
    ...
    reference_text: str = Field(default="", description="Transcript of reference audio - REQUIRED for F5-TTS")
    ...
    cfg_scale: float = Field(default=2.0, ge=0.5, le=4.0, description="F5-TTS: cfg_strength parameter (default 2.0)")
    ddpm_steps: Optional[int] = Field(default=None, description="Obsolete for F5-TTS - maps to nfe_steps if provided")
    nfe_steps: Optional[int] = Field(default=32, ge=1, le=64, description="F5-TTS: nfe_steps parameter (default 32)")
...
def synthesize(req: SynthesizeRequest, request: Request, authorization: Optional[str] = Header(default=None)):
    ...
    # Map parameters: ddpm_steps -> nfe_steps if provided (backward compat)
